...
// 安装包目录
define('INSTALL_PATH', APP_PATH . 'admin' . DS . 'install' . DS);
...
if (is_file($lockFile)) {
    $errInfo = "当前已经安装{$sitename}，如果需要重新安装，请手动移除application/admin/install/install.lock文件";
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $mysqlDatabase = isset($_POST['mysqlDatabase']) ? $_POST['mysqlDatabase'] : 'lanrudb';
    ...
    try {
        ...
        if (!$sql) {
            throw new Exception("无法读取application/admin/install/install.sql文件，请检查是否有读权限");
        }
        $sql = str_replace("`lanru_", "`{$mysqlPrefix}", $sql);
        ...
        if (!$result) {
            throw new Exception("无法写入安装锁定到application/admin/install/install.lock文件，请检查是否有写权限");
        }
        
        $newSalt = substr(md5(uniqid(true)), 0, 6);
        $newPassword = md5(substr(md5($adminPassword), 5, 8) . $newSalt);
        $pdo->query("UPDATE {$mysqlPrefix}admin SET username = '{$adminUsername}' ,password = '{$newPassword}', salt = '{$newSalt}' WHERE username = 'admin'");
        echo "success";
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlDatabase" value="lanrudb" required="">
                </div>
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlPrefix" value="lanru_">
                </div>
            </div>
