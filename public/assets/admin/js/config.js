...
$(function () {
    ...
    $("form.sidebar-form").on("blur", "input[name=q]", function () {
        ...
        if (val != '') {
            $("ul.sidebar-menu li a:not([href^='javascript:;'])").each(function () {
                if ($("span", this).text().indexOf(val) > -1) {
                    html.push('<a href="' + $(this).attr("href") + '">' + $("span", this).text() + '</a>');
                    if (html.length >= 100) {
                        return false;
                    }
                }
    ...
    //快捷搜索点击事件
    $("form.sidebar-form").on('mousedown click', '.menuresult a', function () {
