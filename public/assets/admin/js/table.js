    ...
    api: {
        ...
        bindevent: function (table) {
            ...
            $(toolbar).on('click', Table.config.addbtn, function () {
                ...
                Lanru.api.open(url, '添加', $(this).data() || {});
            });
            // 导入按钮事件
            ...
            $(toolbar).on('click', Table.config.editbtn, function () {
                ...
                $.each(table.bootstrapTable('getSelections'), function (index, row) {
                    ...
                    Lanru.api.open(url, '编辑', $(that).data() || {});
                });
            });
            //清空回收站
            $(document).on('click', Table.config.destroyallbtn, function () {
                var that = this;
                layer.confirm('确定要清空么?', function () {
            ...
            $("tbody", table).dragsort({
                ...
                dragEnd: function (a, b) {
                    ...
                    Lanru.api.ajax(params, function (data, ret) {
            ...
            $(table).on("click", "[data-id].btn-edit", function (e) {
                ...
                Lanru.api.open(url, '编辑', $(this).data() || {});
            });
        ...
        multi: function (action, ids, table, element) {
            ...
            Lanru.api.ajax(options, function (data, ret) {
                var success = $(element).data("success") || $.noop;
                if (typeof success === 'function') {
                    if (false === success.call(element, data, ret)) {
                        return false;
        ...
        events: {
            operate: {
                'click .btn-editone': function (e, value, row, index) {
                    ...
                    var url = options.extend.edit_url;
                    Lanru.api.open(Table.api.replaceurl(url, row, table), '编辑', $(this).data() || {});
                },
        ...
        formatter: {
            ...
            image: function (value, row, index) {
                value = value ? value : '/assets/public/image/blank.gif';
                var classname = typeof this.classname !== 'undefined' ? this.classname : 'img-sm img-center';
                return '<a href="javascript:void(0)" target="_blank"><img class="' + classname + '" src="' + value + '" /></a>';
            },
            images: function (value, row, index) {
                ...
                $.each(arr, function (i, value) {
                    value = value ? value : '/assets/public/image/blank.gif';
                    html.push('<a href="javascript:void(0)" target="_blank"><img class="' + classname + '" src="' + value + '" /></a>');
                });
                return html.join(' ');
            },
            status: function (value, row, index) {
                var custom = {1: 'success', 0: 'gray', 2: 'danger', 3: 'info'};
            ...
            normal: function (value, row, index) {
                ...
                var textArr = {1: '正常', 0: '隐藏'};

                var keys = typeof textArr === 'object' ? Object.keys(textArr) : [];
                var index = keys.indexOf(value);

                var text = index > -1 ? textArr[value] : null;
                var icon = typeof this.icon !== 'undefined' ? this.icon : null;
                if (!color) {
                    color = index > -1 && typeof colorArr[index] !== 'undefined' ? colorArr[index] : 'primary';
                }

                var html = '<span class="text-' + color + '">' + (icon ? '<i class="' + icon + '"></i> ' : '') + text + '</span>';
                if (this.operate != false) {
                    html = '<a href="javascript:;" class="searchit" data-toggle="tooltip" title="点击搜索'+ text +'" data-field="' + this.field + '" data-value="' + value + '">' + html + '</a>';
                }
                return html;
            },
            toggle: function (value, row, index) {
                ...
                return "<a href='javascript:;' data-toggle='tooltip' title='单击可切换' class='btn-change' data-id='"
            ...
            search: function (value, row, index) {
                ...
                return '<a href="javascript:;" class="searchit" data-toggle="tooltip" title="点击搜索'+ value +'" data-field="' + field + '" data-value="' + value + '">' + value + '</a>';
            },
            ...
            flag: function (value, row, index) {
                ...
                $.each(arr, function (i, value) {
                    ...
                    display = typeof that.searchList !== 'undefined' && typeof that.searchList[value] !== 'undefined' ? that.searchList[value] : value.charAt(0).toUpperCase() + value.slice(1);
                    label = '<span class="label label-' + color + '">' + display + '</span>';
                    if (that.operate) {
                        html.push('<a href="javascript:;" class="searchit" data-toggle="tooltip" title="点击搜索' + display + '" data-field="' + field + '" data-value="' + value + '">' + label + '</a>');
                    } else {
                        html.push(label);
                    }
                });
            ...
            datetime: function (value, row, index) {
                ...
                if (isNaN(value)) {
                    return value ? moment(value).format(datetimeFormat) : '无';
                } else {
                    return value ? moment(parseInt(value) * 1000).format(datetimeFormat) : '无';
                }
            },
        ...
        buttonlink: function (column, buttons, value, row, index, type) {
            ...
            $.each(buttons, function (i, j) {
                ...
                if (typeof attr === 'undefined' || attr) {
                    ...
                    url = j.url ? j.url : '';
                    url = typeof url === 'function' ? url.call(table, row, j) : (url ? Table.api.replaceurl(url, row, table) : 'javascript:;');
