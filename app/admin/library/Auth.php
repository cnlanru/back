...
class Auth extends \lanru\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if ($admin['status'] == '0') {
            $this->setError('管理员被禁止');
            return false;
        }
        if (Config::get('site.login_failure_retry') && $admin->loginfailure >= 10 && time() - $admin->updatetime < 86400) {
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $fixedPage = str_replace(".", "/", $fixedPage);
        // 生成菜单的badge
        ...
        $ids = $activeArr = $selected = $referer = [];
        $refererUrl = Session::get('referer');
        $pinyin = new \Overtrue\Pinyin\Pinyin('Overtrue\Pinyin\MemoryFileDictLoader');
        // 必须将结果集转换为数组
        $ruleList = collection(AuthRule::where('status', 1)->where('ismenu', 1)->order('weight', 'desc')->cache("__menu__")->select())->toArray();
        ...
        foreach ($ruleList as $k => &$v) {
            ...
            if ($v['name'] == $fixedPage || stripos($fixedPage,$v['name']) !== false){
                $activeArr[] = $v;
                $ids[] = $v['id'];
            }

            $selected = $v['name'] == $fixedPage ? $v : [];
        ...
        if (Config::get('site.multiplenav')) {
            ...
            $menu = Tree::instance()->getTreeMenu(0, '<li class="@class"><a href="@url@addtabs" addtabs="@id" url="@url" py="@py" pinyin="@pinyin"><i class="@icon"></i> <span>@title</span> <span class="pull-right-container">@caret @badge</span></a> @childlist</li>', $ids, '', 'ul', 'class="treeview-menu"');
